...
const ClientPage: React.FC = () => {
  ...
  const handleAddClient = () => {
    setClient([...clients, { ...newClient, id: clients.length + 1, createdAt: new Date() }]); // ALTERADO
              ...
              {clients.map(
                (
                  ...
                  <tr key={client.id} className="border-b border-gray-200 hover:bg-gray-100">
                    ...
                    <td className="py-3 px-6 text-left">{client.email}</td> {/* ALTERADO */}
                    <td className="py-3 px-6 text-left">{client.address}</td> {/* ALTERADO */}
        ...
        {showAddForm && (
          <div className="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full flex items-center justify-center">
            <div className="bg-white p-5 rounded-lg shadow-lg w-96">
              <h2 className="text-xl font-bold mb-4">Adicionar Novo Cliente</h2> {/* ALTERADO */}
              <input
                type="text"
