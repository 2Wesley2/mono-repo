...
const CATEGORIES: Category[] = ['REFEIÇÕES', 'GERAL', 'BEBIDAS', 'SALGADOS', 'LANCHES'];
...
export const ComandaControlPage: React.FC = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<Category>('REFEIÇÕES');
  ...
  const addItemToComanda = (product: Product) => {
    const existingItem = currentComanda.items.find((item) => item.id === product.id);
  ...
  const calculateTotal = () => {
    return currentComanda.items.reduce((total, item) => total + item.price * item.quantity, 0);
  };

  const filteredProducts = SAMPLE_PRODUCTS.filter((product) => product.category === selectedCategory);
      ...
      <div className="comanda-control">
        ...
        <div className="comanda-content">
          ...
          <div className="total-container">
            ...
            <PaymentButton total={calculateTotal()} onPaymentComplete={handlePaymentComplete} />
          </div>
        </div>
        ...
        <div className="products-section">
          ...
          <div className="products-grid">
            {filteredProducts.map((product) => (
              <button key={product.id} className="product-button" onClick={() => addItemToComanda(product)}>
                {product.name}
              </button>
            ))}
          </div>
